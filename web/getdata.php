<?php
$conn = mysql_connect ("localhost", "temploggning", "temploggning")
			or die (mysql_error());
		mysql_select_db ("temploggning")
...
if(isset($_GET['sensor'])) {	
	...
	if(isset($_GET['usecache']) && $_GET['usecache']) {
		$outStr = readCache($sensor, $from, $period);
	}
	if(!isset($outStr) || !$outStr) {
		if(isset($_GET['period']) && $_GET['period'] == 'latest') {
			$query = "SELECT UNIX_TIMESTAMP(date) as date, temp FROM " . $sensor . " WHERE date = (SELECT MAX( date ) FROM " . $sensor .");";
		} else {
			$query = "SELECT " .
	...
	$result = mysql_query("SELECT name,color FROM sensors ORDER BY name");
	while($row = mysql_fetch_array($result)) {
...
function getCacheFileName($sensor, $from, $period) {
	return 'cacheData/' . $sensor . '_' . $from . '_' . $period;
}
